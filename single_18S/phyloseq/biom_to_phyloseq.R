...
taxa_names(OTU)
taxa_names(TAX)
ps_d_18S <- phyloseq(OTU, TAX, META)

rank_names(ps_d_18S)
get_taxa_unique(ps_d_18S)
sum(otu_table(subset_taxa(ps_d_18S, domain%in%"d__Eukaryota")))/sum(otu_table(ps_d_18S))
sum(otu_table(subset_taxa(ps_d_18S, family%in%"f__Eimeriorina")))/sum(otu_table(ps_d_18S))

#filter by family
ps_d_18S_eimf <- subset_taxa(ps_d_18S, family%in%"f__Eimeriorina")

saveRDS(ps_d_18S, file="/SAN/Susanas_den/gitProj/LabQiime2/single_18S/tmp/deblur_phyloseq_single_18S.RDS")
